...
// Usage: market_data_websocket_per_coin_user_thread_dispatch [seconds]
// Defaults: seconds=30
...
struct CoinFeed {
    ...
    explicit CoinFeed(std::string coin_, slick::stream_buffer_multiplexer &mux_, bool user_thread_dispatch)
        : coin(std::move(coin_))
        , info(hyperliquid::TESTNET_API_URL, mux_, /*skip_ws=*/false, user_thread_dispatch)
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        LOG_INFO("Usage: market_data_websocket_per_coin_user_thread_dispatch [seconds]");
        return 1;
    }

    slick::stream_buffer_multiplexer mux(1u << 16);
    uint64_t cursor = mux.initial_reading_index();

    std::vector<std::unique_ptr<CoinFeed>> feeds;
    feeds.push_back(std::make_unique<CoinFeed>("ETH", mux, true));
    feeds.push_back(std::make_unique<CoinFeed>("BTC", mux, false));
    ...
    // Dispatch message on the main thread
    auto stop_time = std::chrono::steady_clock::now() + std::chrono::seconds(seconds);
    while (std::chrono::steady_clock::now() < stop_time) {
        ...
        for (auto& feed : feeds) {
            auto b = feed->info.dispatch(record.producer_id, reinterpret_cast<const char*>(record.data), record.length);
            if (b && record.producer_id == feeds[1]->info.ws_producer_id()) {
                LOG_ERROR("BTC shouldn't dispatched on this thread");
            }
        }
    }
